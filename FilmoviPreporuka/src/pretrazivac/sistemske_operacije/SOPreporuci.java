...
public class SOPreporuci {
	public static LinkedList<Film> izvrsi(Korisnik k, LinkedList<Korisnik> korisnici){
		int brojac = 0;
		...
		for(int i=0;i<korisnici.size();i++){
			System.out.println(korisnici.get(i).getKoeficijent());
		}
		
		for(int i=0;i<korisnici.size();i++){
			for(int j=0;j<korisnici.get(i).getFilmovi().size();j++){
				if(k.getFilmovi().contains(korisnici.get(i).getFilmovi().get(j))==false){
